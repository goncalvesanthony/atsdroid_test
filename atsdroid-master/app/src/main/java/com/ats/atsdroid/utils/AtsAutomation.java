...
public class AtsAutomation {
    ...
    public AtsAutomation(int port, AtsRunner runner, String ipAddress, Boolean usb, Rect rootBounds) {
        this.usbMode = usb;
        this.port = port;
        this.runner = runner;
    ...
    private void loadApplications() {
        ...
        final List<ResolveInfo> resolvedApps = pkgManager.queryIntentActivities(mainIntent, 0);
            ...
            if (RequestType.APP.equals(req.type)) {
                        ...
                        if (req.parameters.length > 1) {
                            try {
                                ...
                                if (app != null) {
                                    ...
                                    final int childCount = automation.getRootInActiveWindow().getChildCount();
                                    for(int i=0; i<childCount; i++){
                                        final String viewId = automation.getRootInActiveWindow().getChild(i).getViewIdResourceName();
                ...
                if (req.parameters.length > 0) {
                    if (RequestType.START.equals(req.parameters[0])) {
                        ...
                        jsonObject.put("status", "0");
                        DeviceInfo.getInstance().driverInfoBase(jsonObject, rootBounds.bottom);
